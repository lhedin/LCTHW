...
static char* test_optimizations()
{
  size_t N = 2000;
  clock_t start, stop;
  RadixMap* map;
  ...
  for(i = 0; i < 2; i++) {
    ...
    stop = clock();
    printf("Optimization no %d: %"PRIu64" ms \n", i,
           (uint64_t)((stop - start) / (CLOCKS_PER_SEC/1000)));
    RadixMap_destroy(map);
  }
...
char* all_tests()
{
  ...
  mu_run_test(test_optimizations);
