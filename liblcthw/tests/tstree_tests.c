...
char* test_search_exact()
{
    ...
    res = TSTree_search(node, bdata(&test3), blength(&test3));
...
char* test_collect()
{
    DArray* array = DArray_create(0, 10);

    TSTree_collect(node, "T", 1, array);

    char* element = DArray_pop(array);
    mu_assert(strncmp(element, value4, strlen(value4)) == 0,
              "Wrong value in darray! Expected VALUE4.");
    
    element = DArray_pop(array);
    mu_assert(strncmp(element, reverse, strlen(reverse)) == 0,
              "Wrong value in darray! Expected VALUER.");

    element = DArray_pop(array);
    mu_assert(strncmp(element, valueA, strlen(valueA)) == 0,
            "Wrong value in darray! Expected VALUEA.");
            
    element = DArray_pop(array);
    mu_assert(strncmp(element, value2, strlen(value2)) == 0,
            "Wrong value in darray! Expected VALUE2.");
    
    mu_assert(DArray_count(array) == 0, "Darray not empty!");
...
{
    DArray* array = DArray_create(sizeof(char*), 10);

    TSTree_search_suffix(node, "T", 1, array);
    //DArray_print(array, "search suffix:");
