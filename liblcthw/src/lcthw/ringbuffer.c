...
RingBuffer* RingBuffer_create(int length)
{
    ...
    check(buffer, "Could not allocate RingBuffer.");

    buffer->length = length + 1;
    buffer->start = 0;
    buffer->end = 0;
    buffer->buffer = calloc(buffer->length, 1);
    check(buffer->buffer, "Could not allocate buffer.");
...
int RingBuffer_write(RingBuffer* buffer, char* data, int length)
{
    ...
    check(result != NULL, "Failed to write data in to buffer.");
...
void RingBuffer_print(RingBuffer* buffer)
{
    ...
    printf("RingBuffer: start = %d end = %d buffer = ", buffer->start, buffer->end);
