...
static inline BSTreeNode* BSTreeNode_getnode(BSTree* map, BSTreeNode* node, void* key)
{
  int cmp = map->compare(node->key, key);
  ...
  if (cmp == 0) {
    ...
    if (node->left) {
      return BSTreeNode_getnode(map, node->left, key);
    } else {
      return NULL;
    }
  } else {
    if (node->right) {
      return BSTreeNode_getnode(map, node->right, key);
    } else {
      return NULL;
    }
  }
...
void* BSTree_get(BSTree* map, void* key)
{
  if (map->root == NULL) {
    ...
    BSTreeNode* node = BSTreeNode_getnode(map, map->root, key);
