...
char* test_binstr_performance()
{
  ...
  debug("BINSTR COUNT: %lu, END_TIME: %d, OPS: %f", find_count,
...
char* test_find_performance()
{
  ...
  for (j = 0; j < 10; j++) {
    ...
    Stats_sample(st, find_count / elapsed);
    
    debug("FIND COUNT: %lu, END_TIME: %d, OPS: %f", find_count,
    (int) elapsed, (double) find_count / elapsed);
  }
...
char* test_scan_performance()
{
  ...
  debug("SCAN COUNT: %lu, END_TIME: %d, OPS: %f", find_count,
	 (int) elapsed, (double) find_count / elapsed);
