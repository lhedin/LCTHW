/*
 * Excercise 45: A simple TCP/IP Client
 * 
 * Ex of how to run:
 * 
 * Start netcat:
...
int write_some(RingBuffer* buffer, int fd, int is_socket, bool nflag)
{
    int rc = 0;
...
int main(int argc, char* argv[])
{
    ...
    while ((opt = getopt(argc, argv, "n")) != -1) {
        ...
        case 'n':
            ...
            break;
        default:
            fprintf(stderr, "USAGE: netclient [-n] host port.\n");
            return 1;
        }
    }

    check((argc - optind) == 2, "USAGE: netclient [-n] host port");

    socket = client_connect(argv[optind], argv[optind + 1]);
    check(socket >= 0, "connect to %s:%s failed.", argv[optind], argv[optind + 1]);
